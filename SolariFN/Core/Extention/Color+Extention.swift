//
//  Color+Extention.swift
//  SolariFN
//
//  Created by Muchamad Iqbal Fauzi on 13/05/25.
//


import Foundation
import SwiftUI

extension Color {
    init(hex: String) {
        let hex = hex.trimmingCharacters(in: CharacterSet.alphanumerics.inverted)
        var int: UInt64 = 0
        Scanner(string: hex).scanHexInt64(&int)
        let a, r, g, b: UInt64
        switch hex.count {
        case 3:
            (a, r, g, b) = (255, (int >> 8) * 17, (int >> 4 & 0xF) * 17, (int & 0xF) * 17)
        case 6:
            (a, r, g, b) = (255, int >> 16, int >> 8 & 0xFF, int & 0xFF)
        case 8:
            (a, r, g, b) = (int >> 24, int >> 16 & 0xFF, int >> 8 & 0xFF, int & 0xFF)
        default:
            (a, r, g, b) = (255, 0, 0, 0)
        }

        self.init(
            .sRGB,
            red: Double(r) / 255,
            green: Double(g) / 255,
            blue: Double(b) / 255,
            opacity: Double(a) / 255
        )
    }
    
    static let Primary: Color = Color(red: 0, green: 0.76, blue: 0.94)
        
    static let TutorialColor: Color = Color(red: 0.86, green: 0.45, blue: 0.01)
}

extension Color {
    static let crimson = Color(hex: "#DC143C")
    static let greenYellow = Color(hex: "#D5FF5F")
    static let volcanicSand = Color(hex: "#3E3E47")
    static let hollyGlen = Color(hex: "#A2BAB9")
    static let whiteCrest = Color(hex: "#F8F7EF")
    static let fuhciaFever = Color(hex: "#FF5394")
}
